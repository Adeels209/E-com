                                    ...
                                    <div class="card-content collapse show">
                                        <div class="card-body card-dashboard">
                                            ...
                                            <table class="table table-striped table-bordered comma-decimal-place">
                                                ...
                                                <tr class="del" data-id="{{ $role->id }}">
                                                    <td>{{ $role->name }}</td>
                                                    <td>{{ $role->guard_name }}</td>
                                                    <td>{{ $role->created_at->diffForHumans() }}</td>
                                                    <td>{{ $role->updated_at->diffForHumans() }}</td>
                                                    <td style="text-align: center"><a class="message-modal-anchor edit " href="{{ route('role.edit', $role->id) }}"><button class="btn btn btn-outline-primary" id="edit"><i class="ft-edit-2"></i></button></a>
                                                        <button style="margin-left: 20px;" onclick="delFunction('{{$role->id}}')" type="submit" class="btn btn-outline-danger"><i class="ft-delete"></i></button></td>
                                                </tr>
                                                    @endforeach
                                                    @endif
